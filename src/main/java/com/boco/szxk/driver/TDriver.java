...
import cms.impl.GroupSzxkCmsProtocol;
import com.boco.szxk.constant.ConstVarient;
import com.boco.szxk.constant.DriverPubConst;
import com.boco.szxk.tcpcomm.IMessageListener;
import com.boco.szxk.tcpcomm.IPortStatusListener;
import com.boco.szxk.tcpcomm.TTCPComm;
...
import com.boco.szxk.tcpcomm.RequestEncoder;
import com.boco.szxk.tcpcomm.ResponseDecoder;
import com.boco.szxk.constant.DriverConst;
import com.boco.szxk.tcpcomm.MsgDealHandler;
import com.boco.szxk.model.Request;
import com.boco.szxk.model.Response;
...
public class TDriver implements IMessageListener, IPortStatusListener {
        ...
        try {
			switch (sendDriverVarInfo.getDwSendCmd()) {
				case DriverConst.Const_Cmd_PlayLst: //播放表
					...
					GroupSzxkCmsProtocol CmsProtocol = new GroupSzxkCmsProtocol();
					String playlist = CmsProtocol.buildProtocal(sendStr);
					System.out.println("下发播放表--->" + playlist);

					ByteArrayOutputStream baos = new ByteArrayOutputStream();
					//文件更新标志
					baos.write(new byte[]{0x31});
					//文件 帧标记
					baos.write(new byte[]{0x30});

					//播放表文件名
					String playListName = "list\\000.xkl";
					int len = playListName.length();
					String playlistNameLen = String.format("%03d", len);
					//加入播放表文件名长度
					baos.write(playlistNameLen.getBytes("GBK"));
					//加入播放表文件名
					baos.write(playListName.getBytes("GBK"));

					//设置文件偏移
					byte[] connArr = new byte[]{0x30, 0x30, 0x30, 0x30};
					baos.write(connArr);
				...
				case DriverConst.Const_Cmd_Bright: //设备屏幕亮度
					if (sendDriverVarInfo.getSzFuncAbbr().equals("BRIM")) //设置屏幕亮度模式
					{
						...
						try {
							fValue = Integer.parseInt(sendStr);
							Dummy = "0" + CoderUtils.IntTo2SizeString(fValue)
			...
			switch (Cmd) {
				case DriverConst.Const_Cmd_PlayLst://下发播放表
					if (resp.getDataArray().length == 1 && resp.getDataArray()[0] == DriverConst.Const_Answer_Success) {
						//指定播放表
						byte[] ArrSend = "000.xkl".getBytes("GBK");
						Request req = new Request(faddress, DriverConst.Const_Cmd_DisPlayLst, ArrSend);
				...
				case DriverConst.Const_Cmd_ScrOnOff:
					if (resp.getDataArray().length == 3 && resp.getDataArray()[0] == DriverConst.Const_Answer_Success) {
						//发送成功
						RabbitmqCtrlCmdback(businessNo, true);
					} else {
						//发送失败
				...
				case DriverConst.Const_Cmd_Bright:
					if (resp.getDataArray().length == 2 && resp.getDataArray()[0] == DriverConst.Const_Answer_Success) {
						//发送成功
						RabbitmqCtrlCmdback(businessNo, true);
					} else {
						//发送失败
	...
	public void RabbitmqCtrlCmdback(String businessNo,boolean SendFlag) {
		try {
			...
			Identitytemp.setSourceId("collctrsvr_szxk_cms");
			...
			for (TDriverVarInfo driverVarInfo : this.devDriverVarInfo) {
				...
				if (driverVarInfo.getSzFuncAbbr().indexOf("ER") >= 0){
					try{
						...
						int ERValue = Data[bit] - 0x30;//获取设备状态值
						if (ERValue > 0){
							ERValue=0;
						} else {
							ERValue=1;
